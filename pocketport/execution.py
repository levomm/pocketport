...
from .semantics import classify_repository
...
STRATEGY_PRIORITY = {"native": 3, "hybrid": 2, "proot": 1}
NODE_WORKSPACE_MARKERS = {"pnpm-lock.yaml", "package-lock.json", "yarn.lock", "bun.lock", "bun.lockb"}


def _as_execution_component(component: ComponentAssessment) -> ExecutionComponent:
    return ExecutionComponent(component.name, component.role, component.path, list(component.stack), component.score, component.strategy)
...
def _node_run_command(component_root: Path, component: ExecutionComponent) -> str | None:
    ...
    if isinstance(bin_value, str):
        ...
        entries = [(str(name), str(target)) for name, target in bin_value.items() if isinstance(name, str) and isinstance(target, str)]
        if entries:
            chosen = next((entry for entry in entries if entry[0] == component.name), None) or sorted(entries)[0]
            bin_name, bin_target = chosen

    if bin_name and bin_target and (component_root / bin_target).is_file():
        return f"pnpm exec {bin_name}" if package_manager == "pnpm" else f"npx --no-install {bin_name}"
...
def _go_run_command(component_root: Path) -> str | None:
    ...
    if cmd_root.is_dir():
        ...
        if len(candidates) == 1:
            return f"go run ./{candidates[0].relative_to(component_root).as_posix()}"
    return None
...
def _component_key(component: ComponentAssessment, root: Path) -> tuple[int, int, int, int, int, int]:
    ...
    launchable = int(_detect_run_command(_component_root(root, execution), execution) is not None)
    return (
        launchable,
...
def _select_component(report: ScanReport, root: Path) -> ExecutionComponent:
    ...
    if components:
        return _as_execution_component(max(components, key=lambda component: _component_key(component, root)))
    return ExecutionComponent(root.name or "repository", "repository", ".", list(report.stack), report.score, report.strategy)


def _install_root(root: Path, component_root: Path, component: ExecutionComponent) -> Path:
    return _node_workspace_root(root, component_root) if "node" in component.stack else component_root
...
def _component_findings(report: ScanReport, component: ExecutionComponent) -> list[Finding]:
    ...
    return [f for f in report.findings if f.path is not None and (f.path == component.path or f.path.startswith(prefix))]
...
def _compatibility_actions(report: ScanReport, component: ExecutionComponent) -> tuple[list[str], list[str]]:
    ...
    if any(f.kind == "capability" and f.scope == "runtime" for f in findings):
        compatibility.append("runtime-capability")
        notes.append("The program needs OS capabilities that PocketPort cannot safely emulate in stock Termux.")
...
def _agent_skill_install(root: Path) -> list[str]:
    try:
        lines = (root / "README.md").read_text("utf-8", errors="ignore").splitlines()
    except OSError:
        return []
    commands = [line.strip() for line in lines if line.strip().startswith("npx skills add ")]
    if not commands:
        return []
    codex = next((command for command in commands if " -a codex" in command), None)
    return [codex or commands[0]]


def build_execution_plan(report: ScanReport, root: Path) -> ExecutionPlan:
    root = root.resolve()
    profile = classify_repository(root, report.stack)
    component = _select_component(report, root)

    if profile.type == "agent-skill":
        install = _agent_skill_install(root)
        notes = list(profile.notes)
        if not install:
            notes.append("PocketPort identified a SKILL.md artifact but did not infer an installer command from repository metadata.")
        return ExecutionPlan(
            status="usable", target={"platform": "android", "termux": True, "arch": "aarch64"},
            component=component, method="agent-skill", install_directory=".", working_directory=profile.primary_surface,
            install=install, run=[], compatibility=[], notes=notes,
        )

    if profile.type == "desktop-app":
        return ExecutionPlan(
            status="not-direct", target={"platform": "android", "termux": True, "arch": "aarch64"},
            component=component, method="desktop", install_directory=".", working_directory=".",
            install=[], run=[], compatibility=["display-required"],
            notes=list(profile.notes) + ["PRoot alone does not turn a desktop GUI application into a useful phone/Termux target."],
        )

    component_root = _component_root(root, component)
    install_root = _install_root(root, component_root, component)

    if profile.type == "library":
        component_report = ScanReport(str(install_root), list(component.stack), component.score, component.strategy, _component_findings(report, component))
        return ExecutionPlan(
            status="installable", target={"platform": "android", "termux": True, "arch": "aarch64"},
            component=component, method="package", install_directory=_relative(root, install_root), working_directory=component.path,
            install=_native_commands(component_report, install_root), run=[], compatibility=[], notes=list(profile.notes),
        )
    ...
    if component.strategy == "proot":
        return ExecutionPlan(
            ...
            install=_proot_install_commands(), run=[], compatibility=compatibility,
    ...
    return ExecutionPlan(
        status=status, target={"platform": "android", "termux": True, "arch": "aarch64"},
        component=component, method="source", install_directory=install_directory, working_directory=component.path,
        install=install, run=run, compatibility=compatibility, notes=notes,
    )
